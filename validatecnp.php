...
$mysqli = new mysqli("localhost", "root", "", "cnp");
...
function get_cnp_info()
{
    ...
    // insert in db
    if ($stmt = $mysqli->prepare("INSERT INTO user_data (cnp, gen, data_nasterii, varsta, judetul) VALUES (?, ?, ?, ?, ?)")) {
        $stmt->bind_param("issis", $cnp, $gen, $data_nasterii, $varsta, $locul_nasterii);
        $stmt->execute();
